...
// 01. dim(x)
int sphere_dim(arma::mat x){
  return((x.n_rows-1));
}
// 02. inner(x,d1,d2)
double sphere_inner(arma::mat x, arma::mat d1, arma::mat d2){
  return(arma::dot(d1,d2));
}
// 03. norm(x,d)
double sphere_norm(arma::mat x, arma::mat d){
...
// 06. tangent(x,u)
arma::mat sphere_tangent(arma::mat x, arma::mat u){
  return(sphere_proj(x,u));
}
// 07. tangent2ambient
// 08. rand(x)
arma::mat sphere_rand(arma::mat x){
  int n = x.n_rows;
...
// 09. randvec(x)
arma::mat sphere_randvec(arma::mat x){
  int n = x.n_rows;
...
// 10. zerovec(x)
arma::mat sphere_zerovec(arma::mat x){
  int n = x.n_rows;
  arma::mat out(n,1,fill::zeros);
  return(out);
}
// 11. vec(x,u_mat)
arma::mat sphere_vec(arma::mat x, arma::mat u_mat){
  return(u_mat);
}
// 12. mat(x,u_vec)
